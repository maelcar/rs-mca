#!/usr/bin/env python3
"""Verifier for the CAP25 v13 Route-D shared barrier map v2.

Zero-arg, stdlib-only, deterministic. Gates the provenance-gated synthesis note
`experimental/notes/thresholds/cap25_v13_route_d_barrier_map_v2.md` and its
machine map `experimental/data/cap25_v13_route_d_barrier_map_v2.json` against the
real source files in the tree at base 2b1a7e2. v2 supersedes v1
(cap25_v13_route_d_barrier_map.*, base 84b393e); this verifier is independent of
the v1 verifier and reads only the v2 artifacts.
...
Knobs (environment):
  RDMAP2_DATA_DIR   directory holding the JSON (default: <repo>/experimental/data)
  RDMAP2_AS_CAP_GB  address-space soft cap in GiB applied at startup (default: 2)
...
def _apply_as_cap():
    try:
        gb = float(os.environ.get("RDMAP2_AS_CAP_GB", "2"))
    except ValueError:
        gb = 2.0
    if gb <= 0:
        return
...
DATA_DIR = os.environ.get("RDMAP2_DATA_DIR", os.path.join(REPO_ROOT, "experimental", "data"))
JSON_PATH = os.path.join(DATA_DIR, "cap25_v13_route_d_barrier_map_v2.json")
NOTE_PATH = os.path.join(REPO_ROOT, "experimental", "notes", "thresholds",
                         "cap25_v13_route_d_barrier_map_v2.md")
...
def tamper_tests(data, note_text):
    ...
    # 2. wrong constant -> gate (c) must fail (the cross-lineage target_floor,
    #    carried over from v1 and still cited by O412/A0/AUD-TARGETFLOOR)
...
def main():
    ...
    print("Route-D shared barrier-map v2 verifier")
