...
static constexpr int ELL = 17;
static constexpr std::array<int, 23> PRIMES = {
    103, 137, 239, 307, 443, 613, 647, 919, 953, 1327, 2551, 3061,
    3299, 3877, 4217, 4523, 5441, 6733, 9623, 13907, 88469, 126583,
    659941};
...
int determinant4(std::array<std::array<int, 4>, 4> matrix, int p) {
    i64 output = 1;
    for (int column = 0; column < 4; ++column) {
        ...
        const int inverse = int(mod_pow(pivot_value, p - 2, p));
        for (int row = column + 1; row < 4; ++row) {
            const int factor = int(i64(matrix[row][column]) * inverse % p);
            for (int col = column; col < 4; ++col) {
                i64 value = matrix[row][col] - i64(factor) * matrix[column][col];
                value %= p;
                if (value < 0) value += p;
...
int main() {
    ...
    if (roots.size() != 140) return 2;
    std::cout << "ELL17_THREE_SECTOR_SPECTRUM_V1\n";
    for (int prime : PRIMES) {
        ...
        for (const auto& state : states) {
            ...
            for (int index = 0; index < 10; ++index)
                maxima[index] = std::max(maxima[index], sums[index]);
            maximum_fiber = std::max(maximum_fiber, result.maximum_fiber);
        }
        // Include all coefficient vectors without a four-fiber: cap three.
        for (int h = 6; h <= 15; ++h)
    ...
    std::cout << "PASS_ELL17_THREE_SECTOR_SPECTRUM\n";
    return 0;
}
