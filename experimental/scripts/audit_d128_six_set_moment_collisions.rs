//! Independent exhaustive audit of equal-three-moment six-subsets.
//! Uses reverse combination order, generator 3, a different fingerprint,
//! and 32 external partitions.
...
fn fingerprint(key: Key) -> u64 {
    let mut value = ((key.2 as u64) << 32) | key.0 as u64;
    value ^= (key.1 as u64).wrapping_mul(0xd6e8_feb8_6659_fd93);
    value ^= value >> 32;
    value = value.wrapping_mul(0xa076_1d64_78bd_642f);
    value ^= value >> 29;
    ...
    F: FnMut(Key),
{
    ...
    for f in 5..128 {
        for e in 4..f {
            let fe = add(moments[f], moments[e]);
            for d in 3..e {
                let fed = add(fe, moments[d]);
                for c in 2..d {
                    let fedc = add(fed, moments[c]);
                    for b in 1..c {
                        let fedcb = add(fedc, moments[b]);
                        for &first in moments.iter().take(b) {
                            callback(add(fedcb, first));
                            count += 1;
                        }
                    }
                }
        ...
        for e in 4..f {
            ...
            for d in 3..e {
                ...
                for c in 2..d {
                    ...
                    for b in 1..c {
                        let fedcb = add(fedc, moments[b]);
                        for (a, &first) in moments.iter().enumerate().take(b) {
                            callback(add(fedcb, first), Mask::from_indices([a, b, c, d, e, f]));
                            count += 1;
                        }
                    }
                }
...
fn main() {
    let output_path = env::args()
        .nth(1)
        .unwrap_or_else(|| "Q_e6_moment_collision_audit_output.json".to_string());
    let temporary_directory = env::args()
        .nth(2)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("d128_e6_audit_fingerprint_buckets"));
    assert!(
    ...
    let verdict = if other_collision_groups == 0 && disjoint_collision_pairs == 0 {
        "ACCEPT_ALL_E6_COLLISIONS_ARE_FIXED_PAIR_PLUS_T4_BLOCKS"
    } else {
    ...
    writeln!(output, "  \"torus_generator_parameter\": 3,").unwrap();
    writeln!(output, "  \"primary_torus_generator_parameter\": 2,").unwrap();
    writeln!(
        output,
        "  \"enumeration_order\": \"reverse nested combination order\","
    )
    .unwrap();
    writeln!(
        output,
        "  \"fingerprint_function\": \"independent 64-bit mixer\","
    )
    .unwrap();
    ...
    writeln!(output, "  \"scope_guard\": \"Independent exhaustive active-field audit of all equal-first-three-moment six-subsets. The seven-support consequence is a separate exact deduction.\"").unwrap();
    writeln!(output, "}}").unwrap();
}
