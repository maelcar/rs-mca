...
Generated by `experimental/lean/update-blueprint-map.py`.
...
`lean-inventory.json` is the authoritative reverse list of Lean declarations. A declaration with an empty `blueprint_matches` list exists in Lean but is not yet mapped to a CAP25 v13 raw blueprint node.
