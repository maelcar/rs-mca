...
\begin{problem}[entropy-scale inverse theorem for primitive moment-curve sums]\label{prob:entropy-inverse-q}
Prove the following inverse theorem, or identify the new obstruction cell that replaces it.  Let
\[
        V=\{v_y=(\rho(y),\rho(y)y,\ldots,\rho(y)y^{R-1}):y\in T\}\subseteq\B^R,
        \qquad
        \rho(y)^2=y,
\]
be the primitive leaf moment-curve column system after quotient, planted, tangent, extension, Chebyshev, differential-locator, and saturated first-match cells have been removed.  Let \(X\) be a fixed-density singleton or signed-profile selector on \(T\).  If, for some \(r\le(\log n)^A\),
\[
        \Gamma_r^{\rm prim}\ge\exp(\eta nr)
\]
with fixed \(\eta>0\), then a positive-rate set \(U\subseteq T\) is contained in a proper generalized arithmetic progression or coset progression whose affine span has rank \(o(n)\), unless the excess is carried by one of the paid cells: quotient pullback, Chebyshev pullback, planted common block, tangent/common-code-line, extension-valued descent, differential-locator low-defect, or saturation.
\end{problem}
...
Let \(y_1,\ldots,y_t\) be distinct elements of \(T\), with \(t\le R\) and all \(\rho(y_i)\ne0\).  Then the columns
\[
...
This version contains no finite Q, BC, or SP certificate theorem.  The unsafe side is exact and the broad safe anchors are unconditional in their stated ranges.  The former SP input is now a theorem downstream from Q; the raw BC support count is replaced by exact saturation and line-ray identities; and primitive one-parameter BC pencils are paid by the moving-root theorem.  On the asymptotic side, \cref{thm:logmoment-equivalence} proves that primitive Q is exactly a logarithmic collision-moment theorem under \(w\log|\B|/r=o(n)\), and \cref{cor:asymp-q-from-entropy-inverse} reduces the remaining asymptotic frontier proof to the entropy-scale inverse theorem \cref{prob:entropy-inverse-q}.  The full entropy--subfield equality and the adjacent safe inequalities are still not asserted.  The attempted Q closure in \cref{prop:q-exact-target,prop:q-moment-order-floor,prop:bc-not-q,prop:q-not-closed,prob:entropy-inverse-q} identifies the exact missing atom theorem and rules out the current shortcuts: packing is far too weak, fixed moments cannot fit the finite rows, standard polynomial-scale inverse Littlewood--Offord does not directly apply at exponential fiber scale, SP is downstream from Q rather than upstream, and the paid BC theorem applies to one-parameter pencils rather than the high-dimensional boundary-Q slice.  A closed adjacent theorem still requires a row-sharp Q atom bound together with the BC chart-decomposition audit required by \cref{prob:saturated-bc}.
