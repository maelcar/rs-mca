...
> $$\#\{L\in\mathcal A:\ L\mid X^n-1\}\ \le\ P(n)\Big(\binom nj |\mathbb B|^{-s} + 1\Big).$$
...
$$\#\{\text{aperiodic bad slopes at }(n,k,a)\}\ \le\ C\cdot\#\{\text{bad slopes at }(n/2,k/2,\lceil a/2\rceil)\}\ +\ \mathrm{poly}(n)$$
...
1. **Few heavy fibers.** Exact $r=3,4$ collision ledgers (triples/quadruples of split locators with pairwise deep prefix agreement — the pairwise-gcd lattice is literally the sunflower geometry of `sec:v13-l1`, reusable as-is) give $\#\{z: N_w(z)\ge T\cdot\mathrm{avg}\} \le (\text{excess})/T^r$.
