...
\item Use \cref{thm:v13-spi} to state the first SPI chart as eliminant-or-residual, with identically rank-deficient pencils and identically split top charts kept as named residual branches, connected to \cref{lem:singular-pencil} and the eliminant certificates of \cref{sec:aperiodic-hankel-certificates}.
\end{enumerate}

% --------------------------------------------------------------------
% Identity-scale frontier section (strictly supersedes the c=2 proposal)
% --------------------------------------------------------------------

% ======================================================================
% CAP25 v13 identity-scale frontier section.
% Integrated into this single v13 experimental body.  It supersedes the
% deleted standalone cap25_v13_frontier_addendum.tex: the terminal
% graded-prefix scale is the identity scale c=1, not c=2, unless one
% deliberately excludes the identity prefix floor.
% ======================================================================
...
\section{Frontier programme after the identity-scale check}\label{sec:v13f1-frontier}

The proposed terminal-scale addendum correctly identifies a much better unsafe edge than the old optimized rows of \cref{rem:exact-frontier}; however, the route does not actually stop at $c=2$.  The proof of the graded-prefix floor also runs at the identity scale $c=1$.  This gives a strictly better finite certificate in all four deployed rate-$\tfrac12$ rows.  Thus the phrase ``terminal scale'' should mean the identity prefix floor unless the identity map is intentionally excluded from the route.

\subsection{The identity prefix floor}

\begin{lemma}[identity-scale prefix floor]\label{lem:v13f1-identity-prefix-floor}
...
\subsection{Exact deployed certificates at the true terminal scale}
...
\begin{proposition}[identity-scale frontier, exact]\label{prop:v13f1-identity-frontier}
The exact identity-scale prefix-floor certificates are:
\begin{center}\footnotesize
\begin{tabular}{@{}lllllll@{}}
\toprule
row / challenge & $c$ & $m$ & $w$ & $\Delta=m-k$ & certified unsafe range & margins (bits)\\
\midrule
KoalaBear, MCA & $1$ & $1116043$ & $67466$ & $67467$ & $\delta\in[\,\tfrac{981109}{2097152},\tfrac12)=[0.4678292\ldots,\tfrac12)$ & $+25.7\ /\ -5.5$\\
KoalaBear, list & $1$ & $1116046$ & $67470$ & $67470$ & $\delta\in[\,\tfrac{490553}{1048576},1-\rho)=[0.4678278\ldots,1-\rho)$ & $+9.2\ /\ -22.0$\\
Mersenne-31 line round, MCA & $1$ & $1116021$ & $67444$ & $67445$ & $\delta\in[\,\tfrac{981131}{2097152},\tfrac12)=[0.4678397\ldots,\tfrac12)$ & $+10.3\ /\ -20.9$\\
Mersenne-31 line round, list & $1$ & $1116022$ & $67446$ & $67446$ & $\delta\in[\,\tfrac{490565}{1048576},1-\rho)=[0.4678392\ldots,1-\rho)$ & $+28.1\ /\ -3.1$\\
\bottomrule
\end{tabular}
\end{center}
Each printed $m$ passes by exact integer comparison, and $m+1$ fails by exact integer comparison.  These four edges strictly improve the $c=2$ edges
\[
\tfrac{490557}{1048576},\quad
\tfrac{245277}{524288},\quad
\tfrac{490567}{1048576},\quad
\tfrac{245283}{524288}
\]
from the proposed terminal-scale scan, and therefore also supersede the older $c=16/32$ values in \cref{rem:exact-frontier,tab:scanner2}.
\end{proposition}

\begin{proof}
The construction is \cref{lem:v13f1-identity-prefix-floor}.  For the list rows, the displayed list floor directly exceeds the target list budget.  For the MCA rows, apply the floor with $K=k+1$ and use the same deep-point conversion as in \cref{cor:widened-deployed}: the list in $\RS[\F,D,k+1]$ has size exceeding $q/k+1$, and since $m\ge k+1$ the integer admissibility condition $n-m\le n-k-1$ holds.  Thus \cref{thm:A}, \cref{fact:chain}, and \cref{lem:mca-monotone} give the printed MCA-unsafe intervals.

The four certified comparisons are, respectively,
\[
\binom n{1116043}\,k>p^{67466}(p^6+k),\quad
\binom n{1116046}\,2^{128}>p^{67470}p^6,
\]
\[
\binom n{1116021}\,k>(p')^{67444}((p')^4+k),\quad
\binom n{1116022}\,2^{100}>(p')^{67446}(p')^4,
\]
and in each case the same inequality with $m$ replaced by $m+1$ and $w$ by $w+1$ is false.  The comparisons were verified as integer inequalities; the bit margins in the table are only orientation values.  The edge fractions are $1-m/n$.  The strict improvement over $c=2$ is immediate from the displayed fractions.
\end{proof}

\begin{corollary}[superseded deployed sandwich edges]\label{cor:v13f1-sandwich-edges}
In \cref{thm:sandwich2}, the upper edge for the KoalaBear MCA row improves to
\[
        \delta^*_C(2^{-128})\le {981109\over2097152}=0.4678292\ldots,
\]
so the imported sandwich becomes
\[
        \delta^*_C(2^{-128})\in\left[\,{1\over4},\,{981109\over2097152}\,\right]
\]
with the same lower edge as before.  The corresponding list and Mersenne-31 line-round upper edges are the three remaining fractions in \cref{prop:v13f1-identity-frontier}.
\end{corollary}

\begin{proof}
Substitute \cref{prop:v13f1-identity-frontier} into the unsafe side of \cref{thm:sandwich2}; the safe-side inputs are unchanged.
\end{proof}

\subsection{Consequences for the frontier conjecture}

\begin{definition}[frontier function, unchanged]\label{def:v13f1-gstar}
...
\begin{problem}[frontier conjecture, asymptotic and finite forms]\label{prob:v13f1-frontier}
Prove or refute the asymptotic safe-side match
\[
        \delta^*_C(\varepsilon^*)=1-\rho-g^*(\rho,\log_2|\B|)+o(1)
\]
for smooth multiplicative and circle line-round rows at the deployed challenge targets.  The strongest finite deployed version is not literally equivalent to the asymptotic statement: it predicts that the first safe agreement is one more than the best exact unsafe agreement printed in \cref{prop:v13f1-identity-frontier}.  Thus the predicted adjacent pairs are
\[
\begin{array}{c|cc}
\text{row} & \text{proved unsafe agreement }a_0 & \text{conjectured first safe agreement }a_0+1\\ \hline
\text{KoalaBear MCA} & 1116043 & 1116044\\
\text{KoalaBear list} & 1116046 & 1116047\\
\text{Mersenne-31 MCA} & 1116021 & 1116022\\
\text{Mersenne-31 list} & 1116022 & 1116023
\end{array}
\]
with the real closed-ball endpoint convention of \cref{cor:v13-endpoint}.
\end{problem}

\begin{remark}[closure mechanisms after $c=1$]\label{rem:v13f1-closure}
The two ``free'' mechanisms in the proposed addendum should be kept, but with the endpoint changed.  Subgroup-invariant subset selection at a nontrivial scale is absorbed into the composite scale exactly as before; iterating this can only move toward the identity prefix floor, where no nontrivial quotient remains.  The $c=2$, $\sigma=1$ planted hybrid is also dominated by the identity row at the deployed parameters: its best net agreement is at most the $c=2$ agreement minus one, whereas \cref{prop:v13f1-identity-frontier} improves the $c=2$ agreement by $5,2,3,2$ steps in the four rows.  Thus planting remains useful as a structural lower construction, but it is not the deployed frontier certificate once identity prefixes are allowed.
\end{remark}

\subsection{A repaired conditional closing statement}

\begin{proposition}[what the two named inputs actually close]\label{prop:v13f1-closing}
Let $a_0$ be one of the four identity-scale unsafe agreements in \cref{prob:v13f1-frontier}.  An exact finite one-step theorem at the deployed size follows from the exact upper certificate
\[
        U(a_0+1)\le B_*<L(a_0),
\]
where $L$ is the lower staircase supplied by \cref{prop:v13f1-identity-frontier}, $U$ is the complete upper ledger for the same row, and $B_*$ is the challenge budget.  Then \cref{prop:onestep}, equivalently \cref{thm:v13-staircase,thm:v13-corridor}, gives $a_*=a_0+1$.

By contrast, quotient-fiber equidistribution only up to a factor $\operatorname{poly}(n)$, together with the aperiodic band input \cref{prob:band}, is enough for the asymptotic frontier away from an $O(\log n)$ agreement reserve, but it does not by itself prove the finite adjacent pair.  At the deployed size the adjacent fail margins are only $5.5$, $22.0$, $20.9$, and $3.1$ bits in the four identity rows; an unspecified polynomial factor can exceed those margins.
\end{proposition}
...
\begin{remark}[propagation checklist]\label{rem:v13f1-propagation}
If this identity-scale section is promoted into the main Paper D source, propagate the four new fractions to the informal theorem, \cref{rem:exact-frontier}, \cref{thm:sandwich2}, \cref{tab:scanner2}, and the finite-certificate table.  The old $c=16/32$ and proposed $c=2$ rows can remain as cross-checks or historical remarks, but they should no longer be called the optimized frontier.
\end{remark}
