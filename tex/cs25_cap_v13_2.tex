...
\hypersetup{
  pdftitle={Universal Field-Size Caps and a Two-Sided Sandwich for Mutual Correlated Agreement on Smooth Reed--Solomon Domains, Version 13.2},
  pdfauthor={Przemek Chojecki},
  pdfsubject={Mutual correlated agreement and list thresholds for Reed--Solomon codes on smooth domains},
  pdfkeywords={Reed--Solomon codes, proximity gaps, correlated agreement, list decoding, FRI, STARKs}
}
...
\title{Universal Field-Size Caps and a Two-Sided Sandwich\\ for Mutual Correlated Agreement on Smooth Reed--Solomon Domains\\\large Version 13.2}
...
\[
\emca(C,\delta)>\frac1{2k}\Bigl(1-\frac n{|\F|}\Bigr)\ge2^{-86}\gg2^{-128}
\]
throughout the printed near-capacity band, with the same bound for $\eca$ on the integer-admissible subinterval.  The proof is elementary: quotient-locator pigeonhole floors are composed with a self-contained deep-point conversion.  The same mechanism extends to map-smooth domains, Chebyshev line rounds, bivariate circle codes, and rational scale maps relevant to genus-one rows.
...
For the two deployed rate-$\tfrac12$ MCA rows the exact active unsafe edges are row-specific.  For the KoalaBear-sextic row,
\begin{align*}
\delta^*_C(2^{-128})
&\in\left[\frac{349525}{2^{21}},\frac{981105}{2^{21}}\right]
&&\text{unconditionally},\\
\delta^*_C(2^{-128})
&\in\left[\frac14,\frac{981105}{2^{21}}\right]
&&\text{modulo the single classical import \cite{BCIKS20}.}
\end{align*}
For the Mersenne-$31$ circle line-round row,
\begin{align*}
\delta^*_C(2^{-100})
&\in\left[\frac{349525}{2^{21}},\frac{981129}{2^{21}}\right]
&&\text{unconditionally},\\
\delta^*_C(2^{-100})
&\in\left[\frac14,\frac{981129}{2^{21}}\right]
&&\text{with the same import.}
\end{align*}
The corresponding list rows have exact unsafe edges $490553/2^{20}$ and $490565/2^{20}$ and the self-contained Johnson-safe edge printed in \cref{tab:scanner2}.  Genus-one and bivariate-circle transports carry analogous two-sided sandwiches with the unsafe edge supplied by their own rational or Chebyshev scale; they do not inherit the KoalaBear numerical endpoint.
\end{theorem}
...
\paragraph{Roadmap.} \Cref{sec:prelim} fixes definitions and proves the direct deep-point conversion.  \Cref{sec:fiber,sec:main,sec:grand} establish the universal cap and its challenge-level consequences; \cref{sec:subfield,sec:isogeny} treat subfield confinement and map-smooth, circle, and genus-one transports.  \Cref{sec:prize-program,sec:discharge,sec:pincer,sec:answers} develop the staircase, structural ledgers, pincer, explicit witnesses, and finite certificates.  \Cref{sec:frontier-package,sec:capf-threshold} state the active certificate interface and its integer compiler.  \Cref{sec:capff1-frontier,sec:capg-frontier} prove the identity-scale and flexible-budget deployed floors.  \Cref{sec:capg-calculus,sec:capg-rigidity} close the dual-weighted and split-kernel algebra, \cref{sec:capg-floors} proves the aperiodic floors and refutes the original band proposal, and \cref{sec:capg-ledger,sec:capg-subfield} give the exact collision and subfield-normalized ledgers.  \Cref{sec:frontier-next-steps} states the corrected three-bucket closure template and the proper reserve/loss condition; \cref{sec:discussion} summarizes the resulting status.  The appendices retain the longer L1/M1 reductions, proof routes, and computational calibration used to reach the final interface.  Throughout, companion results of \cite{Cho26a,Cho26b} are cited by their printed theorem names rather than numbers, since the numbering of those manuscripts is not yet frozen.
...
\begin{remark}[calibration: quotient floors do not exhaust unsafe mass]\label{rem:quotient-borne}
The quotient-fiber witnesses of \cref{lem:fiber,lem:phi-fiber} are periodic: their agreement supports are unions of complete nontrivial fibers and are charged to the quotient ledger by \cref{thm:stabilizer-partition,thm:fiber-descent}.  The identity scale $c=1$ is different.  The prefix witnesses of \cref{lem:capff1-identity-prefix-floor} need not be periodic, and \cref{thm:capg-aperiodic-floor,cor:capg-asymptotic} later show that at odd agreement every witness in the relevant pole-line fiber is aperiodic and that the resulting slope count can be exponential.  Thus the periodic quotient ledger remains valid, but it cannot support a polynomial aperiodic bound below the entropy--subfield envelope.
\end{remark}

\begin{remark}[Refuted unnormalized band proposal]\label{prob:band}
An earlier draft proposed the following statement.  For a multiplicative or Chebyshev row $C=\RS[\F,D,k]$, one asks for a polynomial $P$, independent of $q$, such that for every agreement
\[
 k+\frac{2n}{N}<a<n-\Bigl\lfloor\frac{n-k}{3}\Bigr\rfloor,
\]
the number of MCA-bad finite slopes of any received line whose witness supports are all aperiodic is at most $P(n)$.  This proposal is retained only as a historical comparison.  It is false below the entropy--subfield envelope by \cref{cor:capg-asymptotic}; no theorem or conditional closure in version 13.2 assumes it.
\end{remark}

\begin{remark}[Normalized aperiodic band formulation]\label{prob:capfr1-normalized-band}
Let $g^*(\rho,\beta)$ be the frontier function of \cref{def:capff1-gstar}, with $\beta=\log_2|\B|$.  The operative aperiodic upper problem begins only at
\[
        a\ge k+g^*(\rho,\beta)n+R(n).
\]
The sub-envelope range is assigned to the unsafe identity-prefix floor.  In an asymptotic theorem the reserve is required to satisfy $R(n)\to\infty$ and $R(n)=o(n)$, and a multiplicative loss $L(n)$ is permitted only when
\[
        \log L(n)=o\!\bigl(R(n)\log|\B|\bigr).
\]
...
\]
No uniform subexponential $L_Q$ exists in the fiber-unsafe sub-band.  Moreover, the identity-scale floors of \cref{thm:capg-aperiodic-floor,cor:capg-asymptotic} rule out a uniform polynomial $L_{\rm ap}$ below the entropy--subfield envelope.  The residual upper ledger is therefore posed only in the normalized range of \cref{prob:capfr1-normalized-band}.
\end{proposition}
...
Together with \cref{thm:weil-lines}, \cref{thm:deep-curve} also covers the multi-parameter samplers into which extension lines decompose, since those are handled over $\F$ directly by \cref{thm:deep-mca}; what the curve-sampler ledger still lacks is only the band-regime aperiodic pivot atlas, i.e.\ the curve instance of the broad band formulation \cref{prob:capfr1-normalized-band}, together with the exceptional vanishing-vector strata already flagged in \cref{sec:aperiodic-hankel-certificates}.
...
\begin{tabular}{@{}p{0.13\linewidth}p{0.22\linewidth}p{0.10\linewidth}p{0.28\linewidth}p{0.15\linewidth}@{}}
\toprule
Row / challenge & Radius range $\delta$ & Verdict & Certified numerator & Source \\
\midrule
\multicolumn{5}{@{}p{0.96\linewidth}@{}}{\emph{KoalaBear sextic}: $n=2^{21}$, $k=2^{20}$, $q=p^6>2^{185.9}$, $\varepsilon^*=2^{-128}$, $\varepsilon^*q\approx2^{57.9}$}\\
\addlinespace[2pt]
...
MCA & $(0.16667,\,0.49218)$ & open & --- & the broad band formulation \cref{prob:capfr1-normalized-band} \\
...
List (any $m$) & $(0.29290,\,0.49609)$ & open & --- & the broad band formulation \cref{prob:capfr1-normalized-band} \\
\midrule
\multicolumn{5}{@{}p{0.96\linewidth}@{}}{\emph{M31 circle line round}: $n=2^{21}$, $k=2^{20}$, $q=p^4<2^{124}$; at $2^{-128}$: $\delta^*=0$ (\cref{prop:small-field}); below: $\varepsilon^*=2^{-100}$}\\
\addlinespace[2pt]
...
The tower-transfer ledger half is already discharged: \cref{thm:stabilizer-partition,thm:fiber-descent,cor:periodic-support-count} were stated and proved for the Chebyshev case alongside the multiplicative case, with the divisor lattice of $D$ replaced by the dyadic scale lattice of the twin coset, exactly as the tower transfer requires; and the broad band formulation \cref{prob:capfr1-normalized-band} is likewise stated for both cases. It remains to treat the genus-one floor. We first extend \cref{def:map-smooth} to rational maps.
...
Deep MCA core & MCA is safe below one third of the distance; singular and rank-drop Hankel branches are removed in that range; the remaining band is the broad band formulation \cref{prob:capfr1-normalized-band}, with rank-pivot and compressed-eliminant certificates. & \cref{sec:discharge-T2,thm:deep-mca} \\
...
\end{tabular}
\end{center}
The certified gap is thus $\approx0.03214\approx2^{-4.96}$ throughout, versus $2^{-7}$ before.  These scale-$16/32$ rows are retained as intermediate general-scale certificates; on the deployed identity-scale rows their endpoints are superseded by \cref{prop:capff1-identity-frontier,prop:capg-moved-frontier}.
\end{remark}
...
\beta:=\log_2|\B| .
\]
Every gap $g<g^*$ is certified at large enough deployed sizes, and \emph{no} gap beyond $g^*$ is certifiable by graded prefix floors at any scale: the frontier is intrinsic to the route, not to our optimization. For $\beta=31$ (both deployed base primes): $g^*(\tfrac12)\approx0.03216$, $g^*(\tfrac14)\approx0.02748$, $g^*(\tfrac18)\approx0.01920$, $g^*(\tfrac1{16})\approx0.01238$. The deployed KoalaBear row achieves $0.03214$ at $n=2^{21}$: the envelope is saturated to three digits already at deployed sizes. Two structural consequences: the left edge of the band in the broad band formulation \cref{prob:capfr1-normalized-band} moves from $k+2n/N$ to $k+g^*n-o(n)$; and the frontier depends on the base field only through $\beta$, so larger base fields \emph{narrow} the certifiable unsafe band --- a quantitative form of the subfield-confinement phenomenon of \cref{sec:subfield}.
\end{remark}

\subsection{The narrowed sandwich}\label{sec:pincer-sandwich}

\begin{theorem}[narrowed row-specific two-sided sandwich]\label{thm:sandwich2}
Let $\delta^*_C(\varepsilon^*)$ be the challenge threshold of \cref{def:dstar}.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(KoalaBear sextic MCA row, $\varepsilon^*=2^{-128}$.)} Self-contained,
\[
\delta^*_C(2^{-128})\in
\left[\frac{349525}{2^{21}},\frac{981105}{2^{21}}\right]
\approx[0.1666665,0.4678273].
\]
With the imported unique-decoding proximity gap of \cref{cor:conditional-half},
\[
\delta^*_C(2^{-128})\in
\left[\frac14,\frac{981105}{2^{21}}\right],
\]
a band of width $0.217828$.
\item \textup{(Mersenne-$31$ circle line-round MCA row, $\varepsilon^*=2^{-100}$.)} Self-contained,
\[
\delta^*_C(2^{-100})\in
\left[\frac{349525}{2^{21}},\frac{981129}{2^{21}}\right]
\approx[0.1666665,0.4678388].
\]
With the same import,
\[
\delta^*_C(2^{-100})\in
\left[\frac14,\frac{981129}{2^{21}}\right],
\]
a band of width $0.217839$.
\item \textup{(General envelope rows.)} For any row with $q\ge2^{128}(n+1)$ and a map-smooth scale lattice, the safe edge is $\max(\lfloor(n-k)/3\rfloor,r_{\mathrm{hJ}})/n$ self-contained, where $r_{\mathrm{hJ}}$ is the largest radius passing \cref{cor:self-contained-safe}, and $\lfloor(n-k)/2\rfloor/n$ with the import.  The unsafe edge is $1-\rho-g$ for every exactly certified $g$, with envelope $g^*(\rho,\log_2|\B|)$ of \cref{rem:entropy-frontier}; the numerical edge is row-specific.
\end{enumerate}
\end{theorem}

\begin{proof}
The lower edges are supplied by \cref{thm:deep-mca} at $r=349525=\lfloor(n-k)/3\rfloor$ and by \cref{cor:conditional-half} at $r=2^{19}$ for the imported clauses; their errors are below the stated row targets as recorded in \cref{tab:scanner}.  The exact upper edges are the two MCA entries of \cref{prop:capg-moved-frontier}.  Part \textup{(iii)} collects the corresponding general safe and unsafe statements without identifying numerical endpoints across different domain families.
\end{proof}

\begin{table}[t]
\centering\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{@{}p{0.18\linewidth}p{0.13\linewidth}p{0.24\linewidth}p{0.18\linewidth}p{0.14\linewidth}@{}}
\toprule
row & challenge & exact unsafe edge & safe, self-contained & safe, with import \\
\midrule
KoalaBear sextic & MCA, $2^{-128}$ & $[981105/2^{21},1/2)$ & $\le349525/2^{21}$ & $\le1/4$ \\
KoalaBear sextic & list, $2^{-128}$ & $\ge490553/2^{20}$ & $\le0.29289$ & --- \\
Mersenne-$31$ circle & MCA, $2^{-100}$ & $[981129/2^{21},1/2)$ & $\le349525/2^{21}$ & $\le1/4$ \\
Mersenne-$31$ circle & list, $2^{-100}$ & $\ge490565/2^{20}$ & $\le0.29289$ & --- \\
\bottomrule
\end{tabular}
\caption{Active deployed scanner output.  The exact unsafe edges are the identity-prefix list certificates of \cref{prop:capff1-identity-frontier} and the flexible-budget MCA certificates of \cref{prop:capg-moved-frontier}; they supersede the earlier scale-$16/32$ endpoints retained in \cref{rem:exact-frontier}.  The self-contained MCA edge is \cref{thm:deep-mca}, the optional imported edge is \cref{cor:conditional-half}, and the list-safe edge is the self-contained Johnson certificate of \cref{thm:johnson-list}.  The corresponding open intervals are printed in \cref{thm:sandwich2}.}
\label{tab:scanner2}
\end{table}

The reduced kernel, restated once. After the pincer, the open MCA band of each deployed row is $(\tfrac14,\ \text{edge})$ with edge $\approx0.4679$, modulo one import whose own proof is independent of everything here; self-contained, it is $(0.1667,\ \text{edge})$, and at rates below $\tfrac14$ the left endpoint improves to half the Johnson radius. Below half the distance the mutual and plain correlated-agreement questions are now provably the same question up to an explicit tangent term (\cref{thm:mca-from-ca}), so the normalized residual ledger \cref{prob:capfr1-normalized-band} is needed only above half the distance, and there the certified unsafe mass presses down to the entropy--subfield envelope $g^*$ (\cref{rem:entropy-frontier}), which the deployed rows already saturate. Closing the band therefore requires one of exactly two kinds of new mathematics: a floor construction whose received words carry more than a graded locator prefix --- anything beating $g^*$ --- or a correlated-agreement theorem crossing half the distance toward capacity, to which \cref{thm:mca-from-ca} would immediately add the mutual layer. This is, for the first time in the program, a two-sided quantitative target with both envelopes made explicit.
...
Let $E$ be a standard-position circle domain as in \cref{lem:stereographic} with $M=2^{21}$ \textup(the deployed shape: $n_c=2M=2^{22}$, $k_c=2^{21}+1$\textup), and let $\F\supseteq\F_p$, $p=2^{31}-1$, be a challenge field with $n_c<q:=|\F|<2^{256}$.
\begin{enumerate}[label=\textup{(\roman*)}]
...
at both rows. The subfield count is $|\Omega\cap\B|\le|\B|=p$.  The obstruction is
projective, not merely affine.  If $\lambda g_\alpha$ were $\B$-valued for some
$\lambda\in\F^\times$, then for two distinct $x,y\in D$ the quotient of its
values would give
\[
        \frac{y-\alpha}{x-\alpha}\in\B.
\]
Writing this quotient as $r\in\B$, the case $r=1$ would force $x=y$, while
$r\ne1$ gives $\alpha=(rx-y)/(r-1)\in\B$, a contradiction.  Hence no common
scalar makes the pair $\B$-valued, so it lies outside the inert class of
\cref{lem:confine}, as \cref{cor:Fvalued} requires of any pair with this
density.
\end{proof}
...
\item \textbf{Identity-prefix list \textup(IP\textup).} Integers $(n,m,K,p,q,t)$ with $D\subseteq\F_p$ and
\[
        \binom nm\,2^t>p^{m-K}q:
\]
mark agreement $m$ list-unsafe at target $2^{-t}$ by \cref{lem:capff1-identity-prefix-floor,prop:capff1-identity-frontier}.
\item \textbf{Flexible-budget MCA \textup(FB\textup).} Integers $(n,m,k,p,q,t)$, $w=m-k-1$, satisfying
\[
\binom nm>p^w\Bigl\lfloor\frac q{2^t}\Bigr\rfloor,
\qquad
\binom{\lfloor q/2^t\rfloor+1}{2}k<q-p,
\qquad
2^t\nmid q:
\]
mark agreement $m$ MCA-unsafe at target $2^{-t}$ by \cref{cor:capg-budget-conversion,prop:capg-moved-frontier}.
\end{enumerate}
\end{definition}
...
\item \textup(Complexity\textup) A binomial certificate of type \textup{(V5)}, \textup{(V6)}, \textup{(V9)}, or \textup{(V10)} is checked with $O(N)$ multiplications of integers of $O(N+\log q)$ bits \textup(the binomial by its product formula, the power $|\B|^w$ by repeated squaring\textup); every other certificate with $O(1)$ arithmetic operations on integers of $O(\log q+\log n)$ bits. A full row is checked in $O(n)$ big-integer multiplications.
...
\cref{prop:capff1-identity-frontier,prop:capg-moved-frontier} & exact prefix pigeonhole, flexible-budget conversion \\
...
\begin{tabular}{@{}p{0.15\linewidth}p{0.25\linewidth}p{0.54\linewidth}@{}}
\toprule
Row / verdict & Certificate tuple & Exact comparison \\
\midrule
KB MCA, FB & $(m,w,t)=(1116047,67470,128)$ & $\binom{2^{21}}{1116047}>p^{67470}\lfloor p^6/2^{128}\rfloor$; $\binom{\lfloor p^6/2^{128}\rfloor+1}{2}2^{20}<p^6-p$; $2^{128}\nmid p^6$ \\
KB list, IP & $(m,w,t)=(1116046,67470,128)$ & $\binom{2^{21}}{1116046}2^{128}>p^{67470}p^6$ \\
M31 MCA, FB & $(m,w,t)=(1116023,67446,100)$ & $\binom{2^{21}}{1116023}>(p')^{67446}\lfloor(p')^4/2^{100}\rfloor$; $\binom{\lfloor(p')^4/2^{100}\rfloor+1}{2}2^{20}<(p')^4-p'$; $2^{100}\nmid(p')^4$ \\
M31 list, IP & $(m,w,t)=(1116022,67446,100)$ & $\binom{2^{21}}{1116022}2^{100}>(p')^{67446}(p')^4$ \\
KB MCA, XH & $(c,m)=(2^{12},258)$, $u=X^{k+2^{13}}|_D$ & $\binom{256}{129}>q/k+1$ \\
KB list, XH & $(c,m)=(2^{14},65)$ & $\binom{63}{32}>2^{-128}q$ \\
M31 MCA, XH & $(c,m)=(2^{12},258)$, $u=T_c^{258}|_D$ & $\binom{256}{129}>q/k+1$ \\
M31 list, XH & $(c,m)=(2^{14},65)$ & $\binom{63}{32}>2^{-100}q$ \\
Both MCA rows, MC & $r=2^{19}$ & $2r\le n-k=2^{20}$ \\
KB safe, HJ & $(r,L_2)=(307121,1001282)$ & $(1+(r+1)L_2)/q<2^{-147}$ \\
KB safe, CH \textsc{import} & $r=2^{19}$ & $n/q<2^{-164}$ \\
M31 safe, CH \textsc{import} & $r=2^{19}$ & $n/q<2^{-102}$ \\
KB profile, PR & $\kappa=2^{54}$ & $\lceil\binom{256}{130}/p\rceil>1+2^{54}qT$ \\
$i$-free circle, RF & $(m,w)=(2^{20}+2^{15}+1,2^{16})$ & $\binom{2^{21}}m>(p')^{2^{16}}2^{256}$ \\
\bottomrule
\end{tabular}
\caption{Selected exact certificates for the deployed rows (\cref{def:certificate-v2}; $p=2^{31}-2^{24}+1$, $p'=2^{31}-1$, and $T=\frac1k(1-\frac nq)$).  The first four rows are the active unsafe endpoints used in \cref{thm:sandwich2,tab:scanner2}; the earlier PF endpoints are intermediate certificates only.  The two \textsc{import} rows are optional half-distance clauses and are the only non-self-contained entries.}
\label{tab:certs}
\end{table}
...
\begin{center}\small
\begin{tabular}{@{}p{0.28\linewidth}p{0.19\linewidth}p{0.21\linewidth}p{0.17\linewidth}@{}}
\toprule
row & proved unsafe $a_0$ & candidate safe $a_0+1$ & deficit floor (bits)\\
\midrule
KoalaBear MCA & 1116047 & 1116048 & \ensuremath{>22.1}\\
KoalaBear list & 1116046 & 1116047 & \ensuremath{>22.0}\\
Mersenne-31 MCA & 1116023 & 1116024 & \ensuremath{>3.2}\\
Mersenne-31 list & 1116022 & 1116023 & \ensuremath{>3.0}\\
\bottomrule
\end{tabular}
\end{center}
...
\text{structured quotient/prefix pullback control},\\
\text{base-field-normalized interior split-pencil control},\\
\text{local primitive shift-pair control}.
\end{gathered}}
\]
...
Every proposed closing statement is to be read through the upper-certificate template of \cref{sec:frontier-next-steps}.  The paper does not claim that the three buckets are already an exhaustive compiler.  The first input prices the boundary prefix and quotient-pullback cells with the necessary mean-plus-one model.  The second prices the interior balanced profiles with the base-field floor of \cref{prop:capg-census-floor}.  The third is local, rather than merely averaged: it bounds the primitive same-prefix neighbours of a fixed support, the quantity required for a max-fiber or per-line bound.  The exact second-moment identity of \cref{thm:capg-second-moment} records the average of these local degrees but does not by itself replace the local input.

\begin{remark}[active closure interface]\label{rem:capg-active-interface}
Let $R(n)\to\infty$ with $R(n)=o(n)$, and let $L(n)$ dominate the multiplicative losses and the number of residual cells in the asymptotic upper ledger.  The reserve form is consumed only under
\[
        \log L(n)=o\!\bigl(R(n)\log|\B|\bigr).
\]
In particular, polynomial loss is compatible with $R\gg\log n$; an unspecified $e^{o(n)}$ loss is not.  At a finite adjacent row, a complete certificate would have to satisfy the single integer inequality displayed in \cref{prop:capg-final-active-package} after proving coverage and coalescing.  No unspecified factor is promoted to a finite certificate.
\end{remark}
...
\[
        ...
        \ge
        \left\lceil \frac{\binom n m}{|\B|^{\,w}}\right\rceil .
\]
...
\begin{center}\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.18\linewidth}crrp{0.35\linewidth}p{0.12\linewidth}@{}}
\toprule
row / challenge & $c$ & $m$ & $w$ & certified unsafe range & approx. $\log_2$ pass / next deficit\\
\midrule
KoalaBear, list & $1$ & $1116046$ & $67470$ &
$\delta\in[\,\tfrac{490553}{1048576},1-\rho)=[0.4678278\ldots,1-\rho)$ & $+9.164\ /\ -22.011$\\
Mersenne-31 line round, list & $1$ & $1116022$ & $67446$ &
$\delta\in[\,\tfrac{490565}{1048576},1-\rho)=[0.4678392\ldots,1-\rho)$ & $+28.113\ /\ -3.073$\\
\bottomrule
\end{tabular}
\end{center}
...
At the active deployed targets,
\[
\begin{array}{rclcrcl}
\delta^*_{C,\mathrm{KB}}(2^{-128})&\le&981105/2^{21},
&\qquad&
\delta^*_{C,\mathrm{M31}}(2^{-100})&\le&981129/2^{21},\\[2mm]
\delta^*_{\mathrm{list},\mathrm{KB}}(2^{-128})&\le&490553/2^{20},
&&
\delta^*_{\mathrm{list},\mathrm{M31}}(2^{-100})&\le&490565/2^{20}.
\end{array}
\]
The first line is supplied by the flexible-budget MCA certificates and the second by the identity-prefix list certificates.  These are the upper endpoints used in \cref{thm:sandwich2,tab:scanner2}.
\end{corollary}

\begin{proof}
The two list inequalities are \cref{prop:capff1-identity-frontier}; the two MCA inequalities are \cref{prop:capg-moved-frontier}.  Monotonicity converts each exact unsafe agreement into the displayed threshold upper bound.
\end{proof}
...
\begin{lemma}[transversality of the entropy--subfield crossing]\label{lem:capff1-transversality}
For fixed $\rho\in(0,1)$ and $\beta>0$, put
\[
F_{\rho,\beta}(g):=\Hb(\rho+g)-\beta g.
\]
Then $\{g\in[0,1-\rho]:F_{\rho,\beta}(g)\ge0\}=[0,g^*]$, the endpoint $g^*=g^*(\rho,\beta)$ lies in $(0,1-\rho)$, and
\[
c_{\rho,\beta}:=-F'_{\rho,\beta}(g^*)
=\beta-\log_2\frac{1-\rho-g^*}{\rho+g^*}>0.
\]
Consequently, for every $R=o(n)$,
\[
nF_{\rho,\beta}\!\left(g^*+\frac{R}{n}\right)
=-c_{\rho,\beta}R+O\!\left(\frac{R^2}{n}\right).
\]
For the deployed rates $\rho\in\{1/2,1/4,1/8,1/16\}$ and every $\beta\ge30$,
$c_{\rho,\beta}\ge\beta-\log_2 15>0.86\beta$.  Thus a multiplicative loss $L(n)$ with
$\log L(n)=o(R(n)\beta)$ is negligible relative to the reserve separation.
\end{lemma}

\begin{proof}
The function $F_{\rho,\beta}$ is strictly concave, with
$F_{\rho,\beta}(0)=\Hb(\rho)>0$ and
$F_{\rho,\beta}(1-\rho)=-\beta(1-\rho)<0$.  Its superlevel set is therefore the interval $[0,g^*]$ with a unique interior boundary point.  Concavity gives
$F'_{\rho,\beta}(g^*)\le(F_{\rho,\beta}(g^*)-F_{\rho,\beta}(0))/g^*<0$, proving $c_{\rho,\beta}>0$.  Taylor's theorem gives the displayed expansion.  Finally,
$\Hb'(x)=\log_2((1-x)/x)$ is decreasing and $\rho+g^*\ge\rho$, so
$\Hb'(\rho+g^*)\le\Hb'(\rho)\le\log_2 15$ on the four deployed rates.
\end{proof}
...
\text{deficit floor (bits)}\\ \hline
\text{KoalaBear MCA} & 1116047 & 1116048 & \ensuremath{>22.1}\\
\text{KoalaBear list} & 1116046 & 1116047 & \ensuremath{>22.0}\\
\text{Mersenne-31 MCA} & 1116023 & 1116024 & \ensuremath{>3.2}\\
\text{Mersenne-31 list} & 1116022 & 1116023 & \ensuremath{>3.0}
\end{array}
\]
...
The standard entropy sandwich gives a quick coarse orientation: for $0<m<M$, taking $\lambda=m/M$ in $1=(\lambda+(1-\lambda))^M\ge\binom Mm\lambda^m(1-\lambda)^{M-m}$ gives $\binom Mm\le2^{M\Hb(m/M)}$, while the largest of the $M+1$ binomial probabilities gives $\binom Mm\ge2^{M\Hb(m/M)}/(M+1)$.  The sharper displayed integer bounds are obtained from the exact binomial ratios in the theorem, with outward-rounded interval evaluation of their logarithms; equivalently, each may be certified by an exact integer power-of-two comparison.  The accompanying verifier is a cross-check, not the proof.
...
\[
>1810769,\quad >1620294,\quad >1462136,\quad \ldots,\quad >63883,\quad >11951
\]
bits at $w=1,2,3,\ldots,20,21$ for the MCA row \textup{(the list row agrees within $4$ bits)}, and failure at $w=22$ by at least $38283$ bits;
\item at the head rungs $m=K+w$ it is nonvacuous for exactly $w\le22$ \textup{(margin at least $+32393$ bits at $w=22$, failure by at least $14136$ bits at $w=23$)};
...
\begin{proof}
Since $\mathcal D$ contains no self-inverse element, $\chi$ is exactly two-to-one on it.  Therefore
\[
\sum_{x\in D}e_p(f(x))
=\frac12\sum_{u\in\mathcal D}e_p(f(\chi(u))).
\]
It suffices to bound each coset sum by $2\deg f\sqrt p$.  Fix the coset $gH$.  As $\UU$ is cyclic and $d'\mid p+1$, the map $v\mapsto gv^{d'}$ carries $\UU$ onto $gH$ with every fiber of size $d'$, so
\[
\sum_{u\in gH}e_p(f(\chi(u)))
=\frac1{d'}\sum_{v\in\UU}e_p\bigl(R(v)\bigr),
\qquad R(v):=f\bigl(\chi(gv^{d'})\bigr).
\]
...
\]
where $T_{d'},W_{d'}\in\F_p[X]$ have degrees $d'$ and $d'-1$ \textup{(with $v^{a}-v^{-a}=(v-v^{-1})W_a(x_v)$)}.  Hence $R$ is an $\F_p$-polynomial function on the affine conic $x^2+y^2=1$, whose $\F_p$-points are exactly the $p+1$ elements of $\UU$: the conic has no rational points at infinity, since $x^2+y^2=0$ has no nonzero solution over $\F_p$.  Over $\overline{\F_p}$ the conic is a projective line with coordinate $u$, and near its two \textup{(conjugate, non-rational)} points at infinity $u\in\{0,\infty\}$ the expansion $\chi(gv^{d'})=\tfrac g2v^{d'}(1+O(v^{-2d'}))$ shows $R$ has exactly two poles, each of order $\deg f\cdot d'$ with nonvanishing leading coefficient.  $R$ is nonconstant.  The $d'$-power map on the cyclic group $\UU$ has image $H$, not all of $\UU$.  Hence $v\mapsto gv^{d'}$ covers $gH$.  The map $\chi$ is injective on $gH$: if $\chi(gh_1)=\chi(gh_2)$, then either $h_1=h_2$ or $g^2h_1h_2=1$, and the second alternative would give $g^2\in H$, contrary to the twin-coset hypothesis.  Thus $\chi(gH)$ has $M=(p+1)/d'$ elements.  Since $\deg f\,d'<p$, one has $\deg f<M$, so constancy of $R$ would make the nonconstant polynomial $f-c$ vanish at more than $\deg f$ points, a contradiction.  Its pole orders lie in $[1,p)$, so $R$ is not of the form $h^p-h+c$, whose pole orders are multiples of $p$.  The standard Weil--Bombieri additive-character bound on a complete nonsingular curve \cite[Ch.~6]{Schmidt76} now gives
\[
...
\begin{enumerate}[label=\textup{(\alph*)}]
\item at the deployed sizes $m=1116023$ \textup{(MCA)} and $m=1116022$ \textup{(list)}, \cref{thm:capff1-collision-circle} is nonvacuous for exactly $w\le10$, with margin lower bounds $>1619021$, $>1322221$, $\ldots$, $>169804$, $>60240$ bits at $w=1,\ldots,10$ and failure at $w=11$ by more than $41998$ bits;
\item at the head rungs $m=K+w$ it is nonvacuous for exactly $w\le11$ \textup{(margin at least $+28846$ at $w=11$)};
...
w_0(c) & 10 & 5 & 2 & 1 & \text{---}\\
w=1\text{ margin (bits)} & >810165 & >331057 & >111115 & >18230 & <-14581
\end{array}
\]
...
\subsection{Finite certificates versus the historical two-input shorthand}

\begin{remark}[finite certificate versus the historical two-input shorthand]\label{prop:capff1-closing}
Let $a_0$ be one of the four active unsafe agreements in \cref{prob:capff1-frontier}.  An exact finite one-step theorem follows only from a complete upper certificate
\[
        U(a_0+1)\le B_*<L(a_0),
\]
where $L$ is the proved lower staircase, $U$ is the full upper numerator for the same row, and $B_*$ is the challenge budget.  Under that assumption \cref{prop:onestep,thm:capf-staircase,thm:capf-corridor} gives $a_*=a_0+1$.

The older shorthand ``\textup{(Q)}+\textup{(A)}'' is retained only as a proof-route grouping.  A polynomial-loss prefix max-fiber estimate supplies the boundary \textup{(Q)} bucket, but an asymptotic closure additionally needs a complete normalized residual certificate: the corrected interior base-field census, local primitive shift-pair control, exhaustive bucket coverage, and support-to-parameter coalescing.  If those data are supplied and their combined loss $L(n)$ satisfies $R(n)\to\infty$, $R(n)=o(n)$, and
\[
        \log L(n)=o\!\bigl(R(n)\log|\B|\bigr),
\]
then the reserve absorbs the loss.  None of these asymptotic statements decides a printed adjacent pair without explicit constants and the single summed integer comparison of \cref{prop:capg-final-active-package}.
\end{remark}
...
\cref{def:mca} with $\gamma$ uniform in $\F$.  For the last sentence run the average over
$\Omega'=\F\setminus(D\cup\B)$.  The line is not projectively $\B$-rational:
if some $\lambda\in\F^\times$ made $\lambda g_\alpha$ $\B$-valued on $D$,
then for distinct $x,y\in D$ the ratio
$g_\alpha(x)/g_\alpha(y)=(y-\alpha)/(x-\alpha)$ would lie in $\B$; the
two-point calculation of \cref{cor:extension-pole-deep-list-floor} would force
$\alpha\in\B$, a contradiction.
\end{proof}
...
\begin{center}\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.23\linewidth}rp{0.16\linewidth}p{0.34\linewidth}p{0.12\linewidth}@{}}
\toprule
row / challenge & $m_1$ & $w=m_1-k-1$ & certified unsafe range &
approx. $\log_2$ pass / next deficit \\
\midrule
...
the two Mersenne-31 rows are now the tight rows (deficits only just above $3.0$ and $3.2$ bits);
...
$\equiv 1\pmod n$ (so $\beta_\nu:=\log_2 p_\nu=O(\log n)$ by Linnik's theorem \cite{IK04}), $B_\nu:=\F_{p_\nu}$, $D_\nu:=\mu_n\subseteq B_\nu^{\times}$; let
...
&\ge n(1-4g^2)-ng\beta_\nu-\log_2(n+1)\\
&\ge n\left(1-\frac{1}{4\beta_\nu^2}-\frac14\right)-\log_2(n+1)\\
&\ge \frac n2.
\end{aligned}
\]
while $\lceil(q_\nu-n)/k\rceil\leq q_\nu/k\leq p_\nu 2^{n/8}=
...
$P(2^{21})>2^{164}$.  For the coefficient-normalized monomial model $P(n)=n^C$ this would require $C\ge8$; no degree lower bound follows for an arbitrary polynomial without controlling its coefficients.  Either way, a value above $2^{164}$ already destroys
...
\item version 13.2 has replaced every operative use of the old proposal in
\cref{thm:conditional-mca,thm:conditional-list} and the closing statements
by \cref{prob:capfr1-normalized-band}; the label \cref{prob:band} is retained
only for the historical statement refuted here, and the finite deployed target
is the new adjacent value of \cref{cor:capg-adjacent-pairs};
...
show why the hypothesis consumed by
\cref{thm:conditional-mca,thm:conditional-list} is the normalized
\cref{prob:capfr1-normalized-band}; the original \cref{prob:band} is retained
only as a refuted historical proposal (see \cref{rem:capg-band}).
...
:=\#\left\{(A,B):
\begin{array}{l}
A,B\text{ are monic of degree }e\text{ and split over }D',\\
...
=\#\left\{(A,c):
\begin{array}{l}
A\text{ is monic of degree }w+1\text{ and split over }D',\\
c\in\F^\times,\quad A-c\text{ splits over }D'
\end{array}
\right\}.
\]
\end{theorem}
...
$a_0+1=1116048,\,1116024$ with next-row deficits exceeding $22.1$ and $3.2$ bits
...
$m':=K-1+d_1$, assume $m'+d_1\le n$, and define
\[
\begin{aligned}
A_{\B}(d_1)&:=\binom n{m'}p^{-(d_1-1)},\\
M_{\B}^{\rm disc}(d_1)&:=\binom{m'}m\ceil{A_{\B}(d_1)},\\
M_{\B}^{\rm soft}(d_1)&:=\binom{m'}m\bigl(1+A_{\B}(d_1)\bigr).
\end{aligned}
\]
Then $M_{\B}^{\rm disc}(d_1)\le M_{\B}^{\rm soft}(d_1)$, and:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \textup{(Boundary profile, $d_1=w'+1$.)}  For every $z\in\B^{w'}$
the witness word $U_z$ of \cref{lem:capff1-identity-prefix-floor} has
minimal shifted lattice degree $d_1(U_z)=w'+1$, and
...
$d_1\bigl(U^{(m')}_{z'}\bigr)=d_1$ and is a balanced determinantal
representation datum in the sense of
\cref{prop:capfr1-detrep,prop:capfp-detrep}.  If $z'^{*}$ indexes a heaviest
level-$m'$ prefix fiber, then its size-$m$ census satisfies
\[
        \Cen\bigl(U^{(m')}_{z'^{*}};\,m\bigr)\ \ge\
        \binom{m'}{m}\,\Bigl\lceil\binom n{m'}\,p^{-(d_1-1)}\Bigr\rceil
        \;=\;M_{\B}^{\rm disc}(d_1).
\]
...
and $56.0$, $43.9$, $57.7$ (KoalaBear, $d_1=w'+2,w'+3,w'+4$), resp.\
$41.0$, $39.2$, $57.7$ (Mersenne-31), at the first interior profiles, against
challenge-field models below one.  The rise at the last displayed profile is the discrete effect of one guaranteed nonempty level-$m'$ prefix contributing all $\binom{m'}m$ of its size-$m$ supports.
\end{proposition}

\begin{proof}
First, $M_{\B}^{\rm disc}\le M_{\B}^{\rm soft}$ follows from
$\ceil{x}\le x+1$ for $x\ge0$.

\textup{(a)}  At the boundary $m'=m$ and the standing hypothesis
$m'+d_1\le n$ gives $m+w'+1\le n$, hence $m+w'<n$.  Suppose a nonzero
$(W,N)\in M_{U_z}$ had shifted degree at most $w'=d_1-1$.  Then
$\deg W\le w'$, $\deg N\le K+w'-1=m-1$, and
$\deg(WP_z-N)\le m+w'<n$.  This polynomial vanishes on $D$, so it is zero.
If $W=0$ then $N=0$; otherwise $N=WP_z$ has degree at least $m$, a
contradiction.  Thus $d_1(U_z)=w'+1$, and
\cref{thm:capfp-Q-unify} gives the census identity.  The fiber lower bound is
the pigeonhole of \cref{lem:capff1-identity-prefix-floor}.
...
The vector $(1,P^{(m')}_{z'})$ has shifted degree $m'-K+1=d_1$.  No nonzero lattice vector $(W,N)$ can have shifted degree at most $d_1-1$: then $\deg W\le d_1-1$, $\deg N\le K+d_1-2=m'-1$, and $\deg(WP^{(m')}_{z'}-N)<n$ by $m'+d_1\le n$.  This polynomial vanishes on $D$, hence is zero.  If $W=0$ then $N=0$; otherwise $N=WP^{(m')}_{z'}$ has degree at least $m'$, contradicting $\deg N\le m'-1$.  Thus $d_1(U^{(m')}_{z'})=d_1$.  The reduced-basis computation of \cref{thm:capfp-Q-unify} then gives
...
distinct $T$ are distinct supports.  Non-$\B$-rationality is projective, not merely affine: if
$\lambda g_\alpha$ were $\B$-valued for some $\lambda\in\F^\times$, the ratio
at two distinct points of $D$ would force $\alpha\in\B$ by the calculation in
\cref{cor:extension-pole-deep-list-floor}.

The deployed violation values are orientation computations of
$\log_2M_{\B}^{\rm disc}(d_1)$ at $(K,m)=(k,1116046)$ resp.\ $(k,1116022)$; the
...
Let $(W_1,N_1,W_2,N_2)$ be a determinantal representation of $\Lambda_D$ as in \cref{prop:capfr1-detrep} with interior balanced profile
\[
        w'+2\le d_1\le\floor{(n-K+1)/2},
        \qquad m'=K-1+d_1.
\]
Write
\[
A_{\B}(d_1)=\binom n{m'}|\B|^{-(d_1-1)},\quad
M_{\B}^{\rm disc}(d_1)=\binom{m'}m\ceil{A_{\B}(d_1)},\quad
M_{\B}^{\rm soft}(d_1)=\binom{m'}m\bigl(1+A_{\B}(d_1)\bigr).
\]
The first quantity after $A_{\B}$ is the unavoidable discrete lower floor of \cref{prop:capg-census-floor}; the second is a smooth upper baseline satisfying $M_{\B}^{\rm disc}\le M_{\B}^{\rm soft}$.  The corrected target census is
\[
\begin{aligned}
&\#\bigl\{(A,B):\ \deg A\le\omega-d_1,\ \deg B\le\omega-d_2,\\
&\hspace{4.5em}AW_1+BW_2\mid\Lambda_D,\ \deg(AW_1+BW_2)=\omega\bigr\}\\
&\qquad\le
L_{\rm BC}(n,d_1)
\max\left\{M_{\B}^{\rm soft}(d_1),\binom n\omega q^{1-w'}\right\}
+E_{\rm BC}(n,d_1),
\end{aligned}
\]
counting monic normalizations.  Applying a ceiling only after multiplying the average prefix-fiber size is invalid: once $A_{\B}(d_1)<1$, one nonempty level-$m'$ prefix still contributes all $\binom{m'}m$ supports.  In the reserve regime one requires $R\to\infty$, $R=o(n)$, and combined logarithmic loss $o(R\log|\B|)$.  In a finite deployed certificate, $L_{\rm BC}$ and $E_{\rm BC}$ are explicit and are summed with every other bucket before comparison with the budget.

The same correction applies \emph{mutatis mutandis} to \cref{prob:capfr1-balanced-core,prob:capfp-balanced}: the interior subfield model is $M_{\B}^{\rm soft}(d_1)$ together with the challenge-field generic term, while $M_{\B}^{\rm disc}(d_1)$ is the unavoidable lower floor.  For \cref{prob:capfr1-rank-one-census,prob:capfp-R1}, the per-line model remains
\[
\max\left\{1,\binom nm|\B|^{-w'},\binom nm q^{-(w'-1)}\right\},
\]
with the middle term achieved by the non-$\B$-rational pole lines of \cref{prop:capg-census-floor}(c).  These models are consistent with the master mean-plus-one target \cref{prob:capfr1-master-flatness} and with the normalized band \cref{prob:capfr1-normalized-band}.
\end{remark}
...
\section{Frontier upper ledger and remaining inputs}\label{sec:frontier-next-steps}

The unsafe side is theorem-level: the identity-prefix floor, the list certificates of \cref{prop:capff1-identity-frontier}, and the flexible-budget MCA certificates of \cref{prop:capg-moved-frontier} supply the lower staircase.  The adjacent safe rows remain conditional.  This section states exactly what their upper certificate must contain.

\begin{definition}[frontier budget and summed upper ledger]\label{def:capg-frontier-ledger}
Fix a row, a sampled-parameter denominator $Q$, target $\varepsilon^*$, agreement $a$, and budget
\[
        B_*:=\floor{\varepsilon^*Q}.
\]
For a received line $\ell$, let $N(\ell,a)$ be its number of bad sampled parameters at agreement at least $a$, and put $N(a):=\max_\ell N(\ell,a)$.  A complete certificate must print an integer $U_{\rm paid}(a)$ together with theorem handles covering the tangent, already-coalesced quotient, extension-lift, common-factor, fixed-dimensional, and other discharged cells; this manuscript does not aggregate those handles into one finished numerator.

A proposed certificate must supply an explicit priority map assigning every unresolved cell to three disjoint classes: boundary prefix/quotient-pullback cells, interior balanced split-pencil cells, and primitive same-prefix cells not assigned to a pullback scale.  Let their line-wise counts be $N_Q(\ell,a)$, $N_{\rm BC}(\ell,a)$, and $N_{\rm sp}(\ell,a)$.  A valid frontier certificate must prove both the coverage statement
\[
        N(\ell,a)\le U_{\rm paid}(a)+N_Q(\ell,a)+N_{\rm BC}(\ell,a)+N_{\rm sp}(\ell,a)
\]
for every line and explicit bounds $N_Q\le U_Q$, $N_{\rm BC}\le U_{\rm BC}$, $N_{\rm sp}\le U_{\rm sp}$.  Thus
\[
        N(a)\le U_{\rm total}(a):=
        U_{\rm paid}(a)+U_Q(a)+U_{\rm BC}(a)+U_{\rm sp}(a).
\]
...
\begin{residualinput}[structured quotient and prefix pullbacks]\label{prob:capg-active-Q}
For every boundary prefix or quotient cell $\nu$ appearing in the compiler, with local parameters $(n_\nu,m_\nu,w_\nu,\B_\nu)$, choose a canonical support in each prefix fiber and let $P_\nu(M)$ be the number of other members related to it by a declared nontrivial pullback scale.  Prove the cell-wise bound
\[
        1+P_\nu(M)
        \le
        \kappa_{Q,\nu}
...
\begin{residualinput}[base-field-normalized interior split pencils]\label{prob:capg-active-BC}
For every interior balanced profile $d_1\ge w'+2$, prove the coverage reduction to the determinantal census of \cref{prob:capg-split-pencil-B} and the explicit bound
\[
C_{\rm BC}(d_1)
\le
\kappa_{{\rm BC},d_1}
...
+E_{{\rm BC},d_1}.
\]
The upper numerator $U_{\rm BC}(a)$ is the sum over the compiler's interior profiles after all overlaps already paid by the priority rule are removed.  The boundary profile $d_1=w'+1$ belongs to \cref{prob:capg-active-Q}, not to this input.
\end{residualinput}

\begin{residualinput}[local primitive shift-pair control]\label{prob:capg-active-shiftpairs}
For an $m$-subset $M\subseteq D$, let
\[
\Delta^{\rm prim}_{w,e}(M)
:=
\#\left\{M':\begin{array}{l}
|M\setminus M'|=e,\ \Phi_w(M')=\Phi_w(M),\\
(M,M')\text{ is not a pullback at any common scale }c\ge2
\end{array}\right\}.
\]
Prove a uniform local bound
\[
        \sum_{e=w+1}^{\min(m,n-m)}\Delta^{\rm prim}_{w,e}(M)
        \le
        \kappa_{\rm sp}\left(1+\binom nm|\B|^{-w}\right)+E_{\rm sp}
\]
for every relevant $M$, and prove that the remaining primitive prefix cells of the compiler inject into these local neighbours.  This local statement is stronger than a bound for the global second moment: \cref{thm:capg-second-moment} gives the sum of the local degrees over $M$, whereas a max-fiber and per-line certificate requires their maximum.  The finite form must give explicit constants and an exact $U_{\rm sp}(a)$.
\end{residualinput}

\begin{remark}[conditional closure template with one budget]\label{prop:capg-final-active-package}
Assume that a future certificate supplies: an explicit cell set and priority map; a proved coverage inequality of \cref{def:capg-frontier-ledger}; support-to-parameter coalescing for every bucket; exact integers $U_{\rm paid},U_Q,U_{\rm BC},U_{\rm sp}$; and the bounds requested in \cref{prob:capg-active-Q,prob:capg-active-BC,prob:capg-active-shiftpairs}.  Under those assumptions the following bookkeeping implications hold.

\begin{enumerate}[label=\textup{(\roman*)}]
\item Let $R(n)\to\infty$ with $R(n)=o(n)$, and let $L(n)$ dominate every multiplicative loss, normalized additive error, and residual-cell count.  If the normalized band begins at $k+g^*n+R(n)$ and
\[
        \log L(n)=o\!\bigl(R(n)\log|\B|\bigr),
\]
...
\item For a finite adjacent row with proved unsafe agreement $a_0$, lower certificate $L(a_0)>B_*$, and a fully instantiated upper certificate, the single comparison
\[
\boxed{
...
\]
forces the first safe agreement to be $a_0+1$ by \cref{thm:capf-staircase,prop:onestep}.  The row margins of \cref{cor:capg-adjacent-pairs} are room for this sum, not separate allowances.
\end{enumerate}
...
\clearpage
\appendix
\section*{Guide to the auxiliary appendices}
\addcontentsline{toc}{section}{Guide to the auxiliary appendices}
...
\[
        ...
        j+t=n-k,
\]
and the M1 problem is to control aperiodic residue-line packings in this $(j,t)$ window --- the residual kernel of the broad band formulation \cref{prob:capfr1-normalized-band}.  The results below are reusable local algebra and combinatorics; they do not prove the full worst-case M1 local limit.
...
The moment calculus is an average over random word pairs.  It does not bound worst-case pairs; those require the aperiodic M1 local limit of the broad band formulation \cref{prob:capfr1-normalized-band}, a Conjecture-F incidence input, or exchange-rigidity input.
\end{remark}
...
\]
a bounded eliminant problem.  The theorem does not classify higher-deficiency SPI charts; those remain inside the broad band formulation \cref{prob:capfr1-normalized-band}.
\end{remark}
...
\]
for the KoalaBear MCA, KoalaBear list, Mersenne-31 MCA, and Mersenne-31 list rows respectively, while the preceding agreements remain unsafe by \cref{prop:capg-moved-frontier} for MCA and \cref{prop:capff1-identity-frontier} for lists.
\end{remark}

\begin{remark}[separation between asymptotic and finite prizes]\label{rem:capfpr-finite-vs-asymp}
The asymptotic prize and the finite adjacent prize should be kept separate.  A factor $n^{C}$ in any residual component costs $C\log_2 n$ bits; at $n=2^{21}$ this is $21C$ bits, larger than several adjacent fail margins in \cref{prop:capff1-identity-frontier,prop:capff1-closing}.  Polynomial losses are compatible with an asymptotic closure only after the complete active certificate, coverage, coalescing, and the reserve conditions of \cref{prop:capg-final-active-package} have been supplied.  A printed adjacent pair needs exact constants, integer rounding, and the single summed budget.
\end{remark}
...
\[
        ...
        \le L_Q(n)\left(1+\frac{\binom n m}{|\B|^w}\right).
\]
The additive $1$ is indispensable once the average fiber is below one.  In an
asymptotic closure $L_Q$ is consumed only with a reserve satisfying
$R\to\infty$, $R=o(n)$, and $\log L_Q=o(R\log|\B|)$.  The finite deployed version must replace $L_Q$ by an
explicit multiplier and additive error whose exact bit cost fits into the
summed one-step upper ledger at $a_0+1$.
\end{remark}

\begin{remark}[historical first-form aperiodic umbrella]\label{prob:capfpr-A}
An earlier organization placed every unpaid primitive aperiodic M1/L1 configuration into one umbrella term and sought
\[
        N_{\rm ap}(A)\le L_A(n)N_{\rm model}(A).
\]
This remains useful as proof-route shorthand only.  To instantiate it above the entropy--subfield envelope, the model must include the corrected discrete/soft base-field census, the proof must control the local primitive same-prefix contribution, and the compiler must prove exhaustive coverage and support-to-parameter coalescing.  An exact deployed version is therefore not a single unspecified integer $U_A$: it is the explicit sum $U_{\rm BC}+U_{\rm sp}$ together with any coverage correction, as required by \cref{def:capg-frontier-ledger}.  The active status statement is \cref{prop:capg-final-active-package}.
\end{remark}

\begin{remark}[historical \textup{(Q)}+\textup{(A)} implication]\label{prop:capfpr-AQ-closing}
If the historical umbrella \textup{(A)} were proved in a form that already included the corrected base-field floors, local primitive control, exhaustive coverage, and parameter coalescing, then \textup{(Q)}+\textup{(A)} would merely repackage the active certificate.  In that case the asymptotic loss is absorbable only for $R\to\infty$, $R=o(n)$, and $\log L=o(R\log|\B|)$, while a finite adjacent theorem still requires
\[
U_{\rm paid}(a_0+1)+U_Q(a_0+1)+U_{\rm BC}(a_0+1)+U_{\rm sp}(a_0+1)
\le B_*<L(a_0).
\]
...
Let $N_{m}:=\binom nm$, $Q_w:=|\B|^w$, and
\[
        S_{m,w}:=\min\{N_m,Q_w\}.
\]
For the probability distribution $\mu_{m,w}$ of $\Phi_{m,w}(M)$ on a uniformly
random $m$-subset, define the discrete-normalized collision gaps
\[
        \Gamma_r(m,w)
        := S_{m,w}^{\,r-1}\sum_{z\in\B^w}\mu_{m,w}(z)^r,
        \qquad r\ge2.
\]
A distribution uniform on $S_{m,w}$ attainable cells has $\Gamma_r=1$, and
$\Gamma_r\ge1$ for every prefix distribution because its support has at most
$S_{m,w}$ points.  This normalization agrees with the usual codomain-uniform
one while $N_m\ge Q_w$, and switches to the injective baseline once $N_m<Q_w$.

\begin{remark}[Fixed-moment collision hierarchy]\label{prob:capfpr-collision-gap}
In the primitive frontier window, prove a quantitative hierarchy strong enough
to imply the first-form quotient input \cref{prob:capfpr-Q}.  A useful target is
\[
        \Gamma_r(m,w)\le n^{Cr}
        \quad\text{for all }2\le r\le c\log S_{m,w},
\]
with constants uniform in the row, or an equivalent tail estimate implying
\[
        \max_z\mu_{m,w}(z)
        \le n^C S_{m,w}^{-1}
        \le n^C\left(N_m^{-1}+Q_w^{-1}\right).
\]
For exact deployment, print the finite moment or direct-tail constants and show
that the induced discrete max-fiber bound fits inside the summed adjacent
ledger.
\end{remark}

\begin{remark}[why fixed low moments are not enough]\label{rem:capfpr-low-moments}
A bound on $\Gamma_r$ for fixed $r$ proves useful bulk equidistribution but does
not by itself imply the max-fiber estimate.  Indeed
\[
        \max_z\mu_{m,w}(z)
        \le \Gamma_r(m,w)^{1/r}S_{m,w}^{-(1-1/r)}.
\]
Thus polynomial control at the effective discrete baseline requires moment
order comparable to $\log S_{m,w}/\log n$, or a separate tail or inverse
theorem.  This is why the collision-gap programme must measure the whole joint
tail, not only the second moment.
\end{remark}
...
\[
        q_j^{(i)}=
        \left(1-\frac{j}{i}\right)q_j^{(i-1)}
        +\frac{j}{i}\,\varepsilon_i q_{j-1}^{(i-1)}.
\]
...
\[
        \frac{\binom{i-1}{j}}{\binom{i}{j}}=1-\frac{j}{i},
        \qquad
        \frac{\binom{i-1}{j-1}}{\binom{i}{j}}=\frac{j}{i}
\]
...
\[
        ...
        =\frac{1}{\binom n m}\sum_{|M|=m}\psi\!\left(\sum_{r=1}^{w}\tau_r\sum_{x\in M}x^r\right),
\]
...
The per-power-sum Weil bound is the wrong endpoint for \textup{(Q)}.  It pays a visible $\sqrt p$ cost per active power-sum direction, and the bounded-prefix base-case theorem \cref{thm:capff1-collision-base} shows exactly how far that method reaches at head depths.  The computations described in \cref{rem:capff1-calibration} indicate that the second-moment refinement over a full period returns the same square-root scale: individual character maxima are controlled by the wrong statistic.  The normalized recursion should therefore be used to measure joint tails and collision gaps, not to claim a uniform character bound strong enough to imply \textup{(Q)} by itself.
\end{remark}
...
The frontier package leaves explicit upper inputs, formalized in \cref{sec:frontier-package,sec:frontier-next-steps}.  This section records proof routes and intermediate reductions for those inputs.  The symbols \textup{(A)} and \textup{(B/Q)} are used locally for the first aperiodic and quotient split; the authoritative status statement is the conditional three-bucket certificate template of \cref{subsec:capg-active-interface}.
...
As a historical umbrella target, an asymptotic \textup{(A)} theorem would prove a row-uniform polynomial bound
\[
        \sup_{f,g}\bigl|\Bad_{\rm ap}^{\rm M1}(f,g;A)\bigr|
        \le n^{C_A}
\]
throughout the frontier band of the broad band formulation \cref{prob:capfr1-normalized-band}, with $C_A$ independent of $q$.  The deployed finite theorem must be sharper: for each printed adjacent candidate $a_0+1$, it must supply an explicit integer
\[
        U_A(a_0+1)
        \ge
...
\[
        U_{\rm paid}(a_0+1)+U_Q(a_0+1)+U_{\rm BC}(a_0+1)+U_{\rm sp}(a_0+1)\le B_* .
\]
...
\[
        H_{c,m,s}^{K}
        \le n^{C_Q}\left(1+\frac{M_{c,m,s}}{|\operatorname{im}\Phi_{c,m,s}^{K}|}\right)
\]
...
\[
        H_{c,m,s}^{K}
        \le
        \Lambda_Q(c,m,s,K)\left(1+\frac{M_{c,m,s}}{|\operatorname{im}\Phi_{c,m,s}^{K}|}\right)+E_Q(c,m,s,K)
\]
with exact integer rounding and with
\[
        \log_2\Lambda_Q+
        \log_2\left(1+\frac{E_Q}{1+M_{c,m,s}/|\operatorname{im}\Phi_{c,m,s}^{K}|}\right)
\]
small enough to fit inside the printed adjacent fail margin after the paid, interior-census, and local shift-pair terms have been added.  Since the smallest deployed adjacent margin in \cref{prop:capff1-identity-frontier,prop:capff1-closing} is only a few bits, an unspecified $\operatorname{poly}(n)$ multiplier is not a finite certificate.
...
This route is the cleanest conceptual proof of \textup{(A)} because it attacks exactly the frontier sentence in the proof guide: the growing-dimensional primitive incidence problem inside the broad band formulation \cref{prob:capfr1-normalized-band}.
...
\[
        U_{\rm atlas}(a_0+1)=\sum_{\mathfrak c}\deg E_{\mathfrak c}+|\mathfrak R_{\rm named}|,
\]
with no asymptotic notation.  Before it can enter a one-step certificate, every term of $U_{\rm atlas}$ must be charged to $U_Q$, $U_{\rm BC}$, $U_{\rm sp}$, or an explicit coverage correction; it is not a fourth unnamed bucket.
...
\item Prove the boundary quotient/prefix theorem first, even with a printed polynomial multiplier, to test the identity-scale formalism above the entropy--subfield crossing.
\item Prove the corrected interior split-pencil census with both $M_{\B}^{\rm disc}$ and $M_{\B}^{\rm soft}$ visible, and separately prove the local primitive shift-pair bound.
\item Define the priority map, prove exhaustive bucket coverage, and prove support-to-parameter coalescing.  Only then combine the asymptotic losses under $R\to\infty$, $R=o(n)$, and $\log L=o(R\log|\B|)$.
\item Return to finite deployment with exact $U_Q,U_{\rm BC},U_{\rm sp},U_{\rm paid}$ and run the one-step compiler of \cref{thm:capf-staircase,thm:capf-corridor}.  Finite closure is claimed only after their exact sum is below $B_*$ at $a_0+1$.
\end{enumerate}
...
The identity-prefix floor changes how the broad band formulation \cref{prob:capfr1-normalized-band} should be read.  Below the entropy--subfield envelope, the identity-scale construction produces exponentially many generic support witnesses, and generic supports are aperiodic in the sense of \cref{def:periodicity-scale} except for a negligible quotient-periodic subfamily.  Therefore the aperiodic band input should be normalized with its left edge at the entropy frontier plus a reserve.

\begin{remark}[developmental restatement of the normalized band]\label{rem:capfr1-normalized-band-dev}
The operative definition is \cref{prob:capfr1-normalized-band}.  The identity-prefix floor explains its left edge: below $k+g^*(\rho,\beta)n$ the first term in the split-locator model is exponentially large, while a reserve $R\to\infty$ with $R=o(n)$ pays a multiplicative loss only when $\log L=o(R\log|\B|)$.  The finite target remains the exact adjacent value $a_0+1$ with every term included in the integer ledger.
\end{remark}
...
The first term in \cref{prob:capfr1-master-flatness} crosses one at exactly the entropy--subfield envelope.  A polynomial-loss version would price the relevant support intersections once the reserve dominates the polynomial cost, but it would not alone prove the frontier: local primitive control, exhaustive coverage, and support-to-parameter coalescing are still required.  Even a complete asymptotic certificate cannot certify a one-step deployed pair without exact constants.
...
\[
        |\Psi_{m,w}^{-1}(z)|
        \le
        \frac{\binom{n}{m-\delta_w+1}}{\binom{m}{\delta_w-1}},
        \qquad
        \delta_w=\left\lceil \frac{w+1}{2}\right\rceil .
\]
...
\[
        S_r(h,T)=\sum_{x\in T}\frac{h(x)x^r}{\Lambda_T'(x)},
        \qquad 0\le r\le w-1.
\]
...
\[
        \sum_{x\in T}\frac{x^d}{\Lambda_T'(x)}=0\quad(0\le d\le m-2),
        \qquad
        \sum_{x\in T}\frac{x^{m-1}}{\Lambda_T'(x)}=1.
\]
...
After all paid cells and common supports have been removed, an intermediate primitive target is
\[
        \#\{T\subseteq D:\ |T|=m,\ T\text{ non-common rank-one for }(u,v)\}
        \le
        L_{\Rone}(n)\max\left(1,\binom n m q^{-(w-1)}\right),
\]
uniformly over primitive affine lines \((u,v)\).  This challenge-field support census is not the active all-strata model: a usable asymptotic theorem must also incorporate the corrected base-field floors and the local primitive shift-pair ledger, with $R=o(n)$ and $\log L_{\Rone}=o(R\log|\B|)$.  The finite deployed form must replace $L_{\Rone}$ by exact constants and an integer numerator assigned through the proved bucket compiler.
\end{remark}

\begin{remark}[scope of the rank-one reduction]\label{cor:capfr1-Q-R1-closing}
The quotient/prefix input \textup{(Q)} and the rank-one support census \(\Rone\) form an intermediate support-counting decomposition.  By \cref{prop:capfr1-slope-elimination}, a proved \(\Rone\) bound injects unpaid primitive slopes into non-common rank-one supports.  It does \emph{not} by itself prove the active frontier: the support model must still be corrected by the base-field-normalized interior floor, the local primitive shift-pair term, exhaustive bucket coverage, and support-to-parameter coalescing.  A valid finite certificate has the authoritative form
\[
        U_{\rm paid}(a_0+1)+U_Q(a_0+1)+U_{\rm BC}(a_0+1)+U_{\rm sp}(a_0+1)
        \le B_*<L(a_0),
\]
as in \cref{prop:capg-final-active-package}.
\end{remark}
...
\subsection{Lattice resolution of \texorpdfstring{$\Rone$}{R1}: near-rational slopes, balanced core, and split pencils}\label{subsec:capfr1-lattice-split}
...
Let $(u,v)$ be an affine line at agreement $m$, write $w=m-K$, and assume
\[
        n-3w\ge m
        \qquad\text{equivalently}\qquad
        m\le\frac{n+3K}{4}.
\]
If two finite slopes $z_1,z_2$ have $d_1(u+z_iv)\le w$ and nonzero census, then $(u,v)$ has a common codeword-pair explanation on at least $n-3w\ge m$ points and hence is not MCA-bad at agreement $m$.  Consequently, outside at most one near-rational slope,
\[
        N_{\mathrm{MCA\text{-}bad}}(u,v;m)
        \le
        1+\#\{z:\ d_1(u+zv)\ge w+1,\ \Cen(u+zv;m)>0\}.
\]
...
\[
        v=\frac{c_1-c_2}{z_1-z_2}+\eta_v,
        \qquad \wt(\eta_v)\le2w,
\]
and then $u$ is within distance $3w$ of a codeword.  Hence the pair has a common codeword-pair explanation on at least $n-3w$ positions, which is at least $m$ by hypothesis.  Therefore no slope is MCA-bad; otherwise at most one near-rational slope exists, and every remaining close slope belongs to the balanced stratum.
\end{proof}
...
\begin{array}{l}
\deg A\le\omega-d_1,\quad \deg B\le\omega-d_2,\\
W_{A,B}\mid\Lambda_D,\quad W_{A,B}\mid N_{A,B}
\end{array}
\right\}
\le e^{o(n)}\max\left\{1,\binom n m q^{-w}\right\},
\]
...
\begin{remark}[scope of the intermediate split-pencil package]\label{cor:capfr1-terminal-package}
The algebraic reductions \cref{lem:capfr1-autodiv,prop:capfr1-detrep,cor:capfr1-balanced-line,prop:capfr1-slope-elimination} show that the balanced primitive support problem factors through a two-generator split-pencil census.  They do not establish the final scale, exhaustive coverage, or parameter coalescing.  The active all-strata certificate must instead use the corrected base-field-normalized numerator of \cref{prob:capg-active-BC}, keep the primitive shift-pair numerator of \cref{prob:capg-active-shiftpairs} separate, and fit all terms into the single budget of \cref{prop:capg-final-active-package}.  In particular, an unspecified polynomial or $e^{o(n)}$ split-pencil loss does not decide any printed adjacent pair.
\end{remark}
...
\[
        N_w(0)
        \le
        \kappa\left(1+\binom n m |\B|^{-w}\right)
\]
with \(\kappa\) and the additive unit included in the printed row budget.
\end{remark}
...
\item \textbf{Immediate.} Use the normalized left edge; run the rung-margin audit; prove and use the slope-elimination reduction; keep the proved anchors \cref{cor:capff1-collision-head,cor:capff1-circle-head,prop:capff1-witness-exact,prop:capff1-composite,cor:capff1-quotient-scales} as proved base cases for \textup{(Q)}.
...
\]
be the prefix map \textup(equivalently the power-sum prefix, by Newton\textup).
The identity floor is
$\max_z|\Phi_{m,w}^{-1}(z)|\ge\lceil\binom nm|\B|^{-w}\rceil$.  Prove the
matching discrete upper bound
\[
        \max_z\bigl|\Phi_{m,w}^{-1}(z)\bigr|
        \le L_Q(n)\left(1+\binom nm\,|\B|^{-w}\right)
\]
throughout the normalized band, with quotient-periodic structure absorbed at
its own scale as in \cref{prop:capff1-composite}.  The asymptotic form is
consumed only when $R\to\infty$, $R=o(n)$, and $\log L_Q=o(R\log|\B|)$; the finite form uses explicit
constants in the summed deployed ledger at $a_0+1$.
\end{remark}

\begin{remark}[Fixed-moment collision hierarchy]\label{prob:capfp-gamma}
Let $N_m=\binom nm$, $Q_w=|\B|^w$, $S_{m,w}=\min\{N_m,Q_w\}$, and let
$\mu_{m,w}$ be the distribution of $\Phi_{m,w}$ on a uniform $m$-subset.  For
$r\ge2$ define
\[
        \Gamma_r(m,w):=S_{m,w}^{\,r-1}\sum_z\mu_{m,w}(z)^r.
\]
Then $\Gamma_r\ge1$, equality holds for the effective uniform baseline, and
$\Gamma_r^{1/r}$ increases to
$R_{m,w}:=S_{m,w}\max_z\mu_{m,w}(z)$.  While $N_m\ge Q_w$, Parseval recovers the
usual codomain-normalized formula; once $N_m<Q_w$, the normalization switches
to the injective baseline.  Prove $\Gamma_r(m,w)\le n^{C_r}$ for each fixed
$r$ in the band and quantify the growth of $C_r$.  The exact conversion is
\[
        R_{m,w}\le \inf_{r\ge2}\Gamma_r(m,w)^{1/r}S_{m,w}^{1/r}.
\]
Thus polynomial discrete max-fiber control requires moment order about
$\log_2S_{m,w}/\log_2n$ at a finite row; second-moment control alone cannot
close the adjacent problem.
\end{remark}

\begin{remark}[historical first-form aperiodic input]\label{prob:capfp-A}
An earlier organization grouped every unpaid aperiodic configuration into one term and sought an inequality
\[
        N_{\mathrm{aper}}(a)\le L(n)N_{\mathrm{model}}(a).
\]
This is retained only to explain the subsequent slope-elimination reductions.  It is not an active closure statement: the model must include the base-field floors of \cref{prop:capg-census-floor}, the primitive same-prefix term must be controlled locally, and an asymptotic loss is usable only when $R=o(n)$ and $\log L=o(R\log|\B|)$.  The authoritative status statement is the conditional certificate template of \cref{sec:frontier-next-steps}.
\end{remark}

\subsection{Proved anchors of the package}

\begin{proposition}[base cases are theorems]\label{prop:capfp-anchors}
At the deployed rows, for every prefix depth $w\le w_0$ of
\cref{cor:capff1-collision-head,cor:capff1-circle-head}
\textup($w_0=21$--$22$ KoalaBear, $10$--$11$ Mersenne-31\textup) and every finite integer $r\ge2$,
\[
        1\le\Gamma_r(m,w)\le
        \bigl(1+2^{-\mathrm{margin}}\bigr)^{r-1},
\]
with the printed margins.  At the max-fiber endpoint,
\[
        1\le R_{m,w}:=S_{m,w}\max_z\mu(z)
        \le1+2^{-\mathrm{margin}}.
\]
At these head depths $N_m\ge Q_w$, so $S_{m,w}=Q_w$ and the effective normalization agrees with the original one.  The exact witness-list, composite-scale, and quotient-cell claims are those of \cref{prop:capff1-witness-exact,prop:capff1-composite,cor:capff1-quotient-scales}.
\end{proposition}

\begin{proof}
The lower bound is the power-mean inequality on a support of size at most
$S_{m,w}$.  For the upper bound put
$R_{m,w}:=S_{m,w}\max_z\mu(z)\le1+2^{-\mathrm{margin}}$ by the two-sided
pricing.  Then
\[
\Gamma_r=S_{m,w}^{r-1}\sum_z\mu(z)^r
\le R_{m,w}^{r-1}\sum_z\mu(z)=R_{m,w}^{r-1}.
\]
The separate max-fiber statement is the displayed bound for $R_{m,w}$; the remaining claims are the quoted results.
\end{proof}
...
\textup{(d)}  Restate \textup{(c)}.  For the last claim, fix one member $M_0$ of a fiber of size $S$.  The map $M\mapsto\mathbf1_M-\mathbf1_{M_0}$ is injective on the other $S-1$ members and produces distinct nonzero balanced ternary words of $K_w$.
\end{proof}
...
\begin{remark}[experimental hierarchy values are not certificate inputs]\label{rem:capfp-gamma-measured}
The Fourier coefficients $\widehat\mu_{m,w}(\tau)$ are computable on small instances by the non-expansive recursion of \cref{rem:capff1-calibration}, and such calculations were useful for choosing conjectures.  No experimental census value is used in a theorem, in the active unsafe certificates, or in the conditional closure template.  The falsifiable asymptotic question remains whether $\log_2\Gamma_r(m,w)=O_r(\log n)$ for fixed $r$ across the normalized band, while the finite adjacent rows require either direct max-fiber control or the much deeper row-dependent hierarchy of \cref{rem:capfp-finite-orders}.
\end{remark}
...
\]
with the finite form fitting under the deployed budget at $a_0+1$.  By \cref{thm:capfp-slope-elim} and the paid compilers, this rank-one census implies the first-form aperiodic input.  It is an intermediate algebraic reduction; the final active split-pencil input is \cref{prob:capg-active-BC}.  The former challenge-field-only calibration is withdrawn because it omits the discrete base-field floors of \cref{prop:capg-census-floor}.  Structure available to attacks remains: the columns are $\B$-valued on $\B$-valued coordinate words (\cref{lem:capfp-functionals}), so \textup{(R1)} is a base-field point count on an explicit determinantal incidence variety, and the twisted vectors $(u(x)/\Lambda_T'(x))\mathbf1_T$, $(v(x)/\Lambda_T'(x))\mathbf1_T$ live in the order-$w$ moment kernel of \cref{prop:capfp-kernel}.  \Cref{lem:capfp-pair-descent} bounds the clique structure of the image of $z(\cdot)$; \textup{(R1)} bounds its domain.  Known unconditional strata: $\B$-rational lines \textup(confinement: at most $p+1$ slopes\textup), and deficiency-one pencils, where \cref{thm:capf-spi} gives a polynomial eliminant with no census needed.
\end{remark}
...
&=\#\left\{(W,N)\in M_U:
\begin{array}{l}
W\textup{ is monic},\quad \deg W=\omega,\\
...
\begin{corollary}[per-line reduction to the balanced core; intermediate form]\label{cor:capfp-line}
Let $(u,v)$ be a line at agreement $m$, write $w=m-K$, and assume $n-3w\ge m$.  If two finite slopes have $d_1(u+zv)\le w$ with nonzero census, then the pair has a common codeword-pair explanation on at least $n-3w\ge m$ points, so no slope is MCA-bad.  Hence
\[
        N_{\mathrm{MCA\text{-}bad}}(u,v;m)
        \le 1+\#\{z:\ d_1(u+zv)\ge w+1,\ \mathrm{cen}(u+zv;m)>0\}.
\]
...
\begin{proof}
By \cref{thm:capfp-dichotomy}(i), two near-rational slopes give $u+z_iv=c_i+\eta_i$ with $\operatorname{wt}\eta_i\le w$.  Subtracting gives a codeword approximation to $v$ with error at most $2w$, and substituting back gives one for $u$ with error at most $3w$.  Their common agreement has size at least $n-3w\ge m$.  If there are not two such slopes, at most one lies outside the balanced stratum; slopes with zero census are not close at agreement $m$.
\end{proof}
...
The shifted-reduced bases of the lattices $M_U$ are precisely the quadruples
$(W_1,N_1,W_2,N_2)$ satisfying
\[
...
For all $r\ge2$, let
\[
        R_{m,w}:=S_{m,w}\max_z\mu_{m,w}(z),
        \qquad \theta_{m,w}:=\log_2S_{m,w}.
\]
Then
\[
        \frac{\log_2\Gamma_r}{r-1}
        \le \log_2R_{m,w}
        \le \frac{\log_2\Gamma_r+\theta_{m,w}}{r}.
\]
Consequently, when $\theta_{m,w}=O(n)$, the asymptotic discrete-flatness bound
$R_{m,w}\le e^{o(n)}$ holds if and only if
$\Gamma_r(m,w)\le e^{o(n)}$ for every fixed $r$.  Deciding a finite row with
margin $\Delta$ bits by moments alone requires order on the scale
$\theta_{m,w}/\Delta$.
\end{proposition}

\begin{proof}
The first inequality follows from
$\Gamma_r\le R_{m,w}^{r-1}$.  For the second,
\[
\max_z\mu(z)\le\left(\sum_z\mu(z)^r\right)^{1/r}
=\Gamma_r^{1/r}S_{m,w}^{-(r-1)/r},
\]
so $R_{m,w}\le(\Gamma_rS_{m,w})^{1/r}$.  For the equivalence, fix an
arbitrarily small $\varepsilon>0$, take a sufficiently large constant $r$, and
use $\theta_{m,w}=O(n)$ in the right inequality; the converse is the left
inequality.
\end{proof}
...
\hline
\text{KoalaBear MCA} & 67470 & 22.197 & 9.42\cdot10^4 \\
\text{KoalaBear list} & 67470 & 22.011 & 9.50\cdot10^4 \\
\text{Mersenne-31 MCA} & 67446 & 3.259 & 6.42\cdot10^5 \\
\text{Mersenne-31 list} & 67446 & 3.073 & 6.80\cdot10^5 .
\end{array}
\]
...
Thus one possible proof route for the boundary-prefix bucket rests on \emph{fixed-order} collision counts --- for each constant $r$, the number of $r$-tuples of degree-$\omega$ divisors of $X^n-\beta$ with pairwise equal top-$(w{+}1)$ coefficients must not exceed $e^{o(n)}\binom nm^{\,r}S_{m,w}^{-(r-1)}$ --- together with an interior split-pencil bound whose actual loss is checked against the reserve condition $\log L=o(R\log|\B|)$.  The notation $e^{o(n)}$ alone does not supply that check.  Fixed moments are exact algebraic objects \textup(via \cref{prop:capfp-kernel}, weighted counts of $\{0,\pm1\}$-configurations in the moment kernel\textup), the most attackable known form of the input.

\begin{proposition}[character expansion of the split-divisor indicator]\label{prop:capfp-Q-smooth}
Let $D=\alpha H\subseteq\F_p^\times$, where $H$ has order $n$, and put $r=(p-1)/n$.  Let $H^\perp$ be the $r$ multiplicative characters of $\F_p^\times$ that are trivial on $H$, and extend every one of them, including the trivial character, by $\chi(0):=0$.  Then for every $x\in\F_p$,
\[
        \Phi_D(x):=\mathbf1_D(x)
        =\frac1r\sum_{\chi\in H^\perp}\chi(x\alpha^{-1}).
\]
If $D$ is the root set of $X^n-\beta$ and $W$ is monic, define
\[
I_D(W):=
\begin{cases}
\displaystyle\prod_{i=1}^{d}\Phi_D(\xi_i),
&\text{if $W$ splits squarefree over $\F_p$ with roots $\xi_1,\ldots,\xi_d$,}\\
0,&\text{otherwise.}
\end{cases}
\]
Then $\mathbf1\{W\mid X^n-\beta\}=I_D(W)$.  For a split squarefree
polynomial $W$ of degree $d$, choosing any ordering $\xi_1,\ldots,\xi_d$ of its roots gives
\[
\prod_{i=1}^d\Phi_D(\xi_i)
=
 r^{-d}\sum_{(\chi_1,\ldots,\chi_d)\in(H^\perp)^d}
 \prod_{i=1}^d\chi_i(\xi_i\alpha^{-1}).
\]
...
\begin{remark}[historical Q/A grouping and the active template]\label{prop:capfp-closing}
The labels \textup{(Q)} and \textup{(A)} describe an earlier two-way grouping: boundary prefix fibers versus the remaining aperiodic census.  They are useful proof-route coordinates, but they are not a complete closure theorem.  The boundary bucket may be attacked through fixed-order collision bounds, and the interior aperiodic bucket through the balanced split-pencil reductions above; neither supplies the local primitive same-prefix maximum, an exhaustive priority assignment, or support-to-parameter coalescing.

Accordingly, the four adjacent-pair predictions follow only from a fully instantiated certificate of \cref{sec:frontier-next-steps}, including the corrected discrete/soft base-field models and the single summed finite budget.  Failure of a candidate certificate may still locate a heavy prefix fiber, a large interior split-pencil census, a large local primitive shift-pair degree, or a coverage/coalescing defect.  The row-dependent moment orders of \cref{rem:capfp-finite-orders} are calibration for one possible boundary proof route, not a substitute for the complete certificate.
\end{remark}
...
\bibitem[Sch76]{Schmidt76}
W.~M.~Schmidt.
\newblock \emph{Equations over Finite Fields: An Elementary Approach}.
\newblock Lecture Notes in Mathematics, Vol.~536. Springer, 1976.
